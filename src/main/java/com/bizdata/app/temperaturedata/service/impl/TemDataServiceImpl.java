...
public class TemDataServiceImpl implements TemDataService {
    ...
    @Autowired
    public TemDataServiceImpl(TemDataRepository temDataRepository, MagUserService magUserService) {
        ...
        return new Specification<TemData>() {
            ...
            public Predicate toPredicate(Root<TemData> root, CriteriaQuery<?> criteriaQuery,
                ...
                if (StringUtils.isNotBlank(searchTemDataVO.getProfileId())) {
                    String userId = magUserService.getIdByAccountnum(Integer.valueOf(searchTemDataVO.getProfileId()));
                    if (null != userId) {
                        in.value(userId);
                        list.add(in);
                    }
                ...
                if (null != searchTemDataVO.getStarttime()) {
                    ...
                    list.add(
                            criteriaBuilder.greaterThanOrEqualTo(root.get("date"), searchTemDataVO.getStarttime()));
                }
                // 如果结束时间存在
                if (null != searchTemDataVO.getEndtime()) {
                    list.add(criteriaBuilder.lessThanOrEqualTo(root.get("date"), searchTemDataVO.getEndtime()));
                }
