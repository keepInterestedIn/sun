...
public class BootController {
    ...
    public ResultVO del(String id) {
        ...
        if(boot.getBootOnlineStateEnum()==BootOnlineStateEnum.ONLINE){
        	boot.setBootOnlineStateEnum(BootOnlineStateEnum.OFFLINE);
        }else {
        	boot.setBootOnlineStateEnum(BootOnlineStateEnum.ONLINE);
        }
        if (bootService.save(boot)) {
            resultVO = ResultUtil.create(0, "修改启动页状态成功!");
        } else {
            resultVO = ResultUtil.create(-1, "修改启动页状态失败!");
        }
        return resultVO;
    }
    ...
    public ModelAndView readOne(String id) {
        Boot boot = bootService.findOne(id);
      //编辑时转义
        boot = StringUnescapeHtml.unescapeHtml(boot);
        ModelAndView mv = new ModelAndView();
        mv.addObject("url", fileUploadConfig.getServerPrefix());
        mv.addObject("boot", boot);
        mv.setViewName("app_page/boot/edit");
        return mv;
    }
}
