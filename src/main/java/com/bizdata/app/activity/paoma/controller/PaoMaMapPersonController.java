...
public class PaoMaMapPersonController {
	...
	public JpaPageResultVO<PaoMaDetails, PaoMaDetails> list(JqGridPageVO jqGridPageVO, JqGridSortVO jqGridSortVO,
		...
		// List<PaoMaDetails> paoMaDetails =
		// paoMaDetailsService.findByActivityId(paoMaMapPersonVO.getId());
		System.out.println(paoMaMapPersonVO.getId());
		...
		Page<PaoMaDetails> paoMaDetail = paoMaDetailsService.findAllByPage(jpaPageParamVO, jpaSortParamVO,
				paoMaMapPersonVO);
		// 补充缺少的数据
		for (PaoMaDetails temp : paoMaDetail) {
			String activityId = paoMaMapPersonVO.getId(); // 活动id
			String userId = temp.getUserId();
			MagUser user = magUserService.findOne(userId);
			...
			if (userInfo != null) {
				...
				if (userInfo.getAddress() != null) {
					...
					if (userInfo.getDetailedAddress() != null) {
						address += userInfo.getDetailedAddress();
					}
					temp.setAddress(address);
				}
			}
			Integer ranking = paoMaMapPersonServiceImpl.getRankByActivityIdAndUserId(activityId, userId); // 获取排名
			temp.setRanking(ranking);
			if (null != ranking) {
				String prizeName = paoMaDetailsService.findPrizeName(activityId, ranking);
				temp.setPrizeName(prizeName);
			}
		}
		...
		// Page<PaoMaMapPerson> paoMaMapPerson =
		// paoMaMapPersonServiceImpl.findAllByPage(jpaPageParamVO,
		// jpaSortParamVO,
		// paoMaMapPersonVO);
