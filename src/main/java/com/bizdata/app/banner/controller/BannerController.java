...
public class BannerController {
    ...
    public ModelAndView editView(String id) {
        ...
        //编辑时转义
        banner = StringUnescapeHtml.unescapeHtml(banner);
        if(null == banner.getGotoChildEnum()) {
        	banner.setGotoChildEnum(GotoChildEnum.NULL);
        }
        ModelAndView mv = new ModelAndView();
        mv.addObject("url", fileUploadConfig.getServerPrefix());
        mv.addObject("banner", banner);
        mv.setViewName("app_page/banner/edit");
        return mv;
    }
    ...
    public ResultVO del(String id) {
        ResultVO resultVO;
        Banner banner = bannerService.findOne(id);
        if("2".equals(banner.getState())||null==banner.getState()){
        	banner.setState("1");
        }else if("1".equals(banner.getState())){
        	banner.setState("2");
        }
        if (bannerService.save(banner)) {
            resultVO = ResultUtil.create(0, "Banner状态修改成功!");
        } else {
            resultVO = ResultUtil.create(-1, "Banner状态修改失败!");
        }
        return resultVO;
    }
}
