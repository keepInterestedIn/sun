...
public class MagUserServiceImpl implements MagUserService {
	...
	public Page<MagUser> findAllByPage(JpaPageParamVO jpaPageParamVO, JpaSortParamVO jpaSortParamVO,
		...
		// return
		// magUserRepository.findAll(jpaPageParamVO.getPageable(jpaSortParamVO.getSort()),
		// searchMagUserVO.getUserName());
		// }
//		Page<Magnar> magnars = magUserRepository.findByCity(
//				jpaPageParamVO.getPageable(jpaSortParamVO.getSort()));
//		for (Magnar magnar : magnars) {
//			System.out.println(magnar.getMobile());
//		}
//		magUserRepository.findAll(jpaPageParamVO.getPageable(jpaSortParamVO.getSort()),
//				magUser);
		
//		Page<UserProfile> t = userProfileService.findAll(listWhereClause(searchMagUserVO,""),
//				jpaPageParamVO.getPageable(jpaSortParamVO.getSort()));
...
//	private Specification<UserProfile> listWhereClause(SearchMagUserVO searchMagUserVO, String string) {
//		return new Specification<UserProfile>() {
//			@Override
//			public Predicate toPredicate(Root<UserProfile> root, CriteriaQuery<?> criteriaQuery,
//					CriteriaBuilder criteriaBuilder) {
//				List<Predicate> list = new ArrayList<>();
//
//				// 如果用户ID不为空
//				if (StringUtils.isNotBlank(searchMagUserVO.getUserId())) {
//					list.add(criteriaBuilder.equal(root.get("accountnum").as(String.class),
//							searchMagUserVO.getUserId()));
//				}
//				Join<UserProfile, MagUser> companyJoin = root.join(root.getModel().getSingularAttribute("accountId",MagUser.class), JoinType.INNER);
//				if (StringUtils.isNotBlank(searchMagUserVO.getUserName())) {
//					list.add(criteriaBuilder.like(companyJoin.get("nickName"),
//							"%" + searchMagUserVO.getUserName() + "%"));
//				}
//				// 如果手机号不为空
//				if (StringUtils.isNotBlank(searchMagUserVO.getUserPhone())) {
//					System.out.println(searchMagUserVO.getUserPhone());
//					list.add(criteriaBuilder.like(root.get("mobile").as(String.class),
//							"%" + searchMagUserVO.getUserPhone().toString() + "%"));
//				}
//
//				// 如果开始时间存在
...
//					list.add(criteriaBuilder.lessThanOrEqualTo(root.get("createTime"), searchMagUserVO.getEndTime()));
//				}
//
//				// 如果注册IP存在
//				if (StringUtils.isNotBlank(searchMagUserVO.getRegisterIp())) {
//					list.add(criteriaBuilder.equal(root.get("accountSource").as(String.class),
//							searchMagUserVO.getRegisterIp()));
//				}
//
//				// 如果渠道号存在
//				if (null != searchMagUserVO.getDitchNo()) {
//					list.add(criteriaBuilder.equal(root.get("ditchNo"), searchMagUserVO.getDitchNo()));
//				}
//
//				// 如果状态存在
//				if (null != searchMagUserVO.getState()) {
//					list.add(criteriaBuilder.equal(root.get("accountStatus"), searchMagUserVO.getState()));
//				}
//
...
//	}
	...
	private Specification<MagUser> listWhereClause(SearchMagUserVO searchMagUserVO) {
		return new Specification<MagUser>() {
			...
			public Predicate toPredicate(Root<MagUser> root, CriteriaQuery<?> criteriaQuery,
				...
				if (StringUtils.isNotBlank(searchMagUserVO.getUserName())) {
					In<String> in = criteriaBuilder.in(root.get("id"));
					List<UserProfile> userProfiles = userProfileService.findAllByNickName(searchMagUserVO.getUserName());
					//Join<MagUser, UserProfile> companyJoin = root.join(root.getModel().getSingularAttribute("id",UserProfile.class), JoinType.INNER);
					if(null != userProfiles && userProfiles.size() > 0) {
						for(UserProfile userProfile : userProfiles) {
							if(null != userProfile.getAccountId() && !"".equals(userProfile.getAccountId())) {
								in.value(userProfile.getAccountId());
							}
						}
					} else {
						in.value("''");
	...
	public String getMobileById(String id) {
		return magUserRepository.getMobileById(id);
	}

	@Override
	public List<String> findIdByMobile(String mobile) {
		return magUserRepository.findIdByMobile(mobile);
	}
}
