		...
		return new Specification<Feedback>() {
			...
			public Predicate toPredicate(Root<Feedback> root, CriteriaQuery<?> criteriaQuery,
                ...
                if (null != searchFeedbackVO.getStartTime()) {
                    list.add(criteriaBuilder.greaterThanOrEqualTo(root.get("backdate").as(Date.class), searchFeedbackVO.getStartTime()));
                }

                //如果结束时间存在
                if (null != searchFeedbackVO.getEndTime()) {
                    list.add(criteriaBuilder.lessThanOrEqualTo(root.get("backdate").as(Date.class), searchFeedbackVO.getEndTime()));
                }
