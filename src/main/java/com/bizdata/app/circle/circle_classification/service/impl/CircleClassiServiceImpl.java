...
public class CircleClassiServiceImpl implements CircleClassiService {
	...
	public boolean save(CircleClassi circleClass) {
		...
		try {
			...
			//圈子分类状态为删除,则将圈子和话题状态置为删除
			if(circleClass.getStateEnum()==StateEnum.DELETED){
				circleTopicService.setStateByClassId(circleClass.getId(),"3");
				circleService.setStateByClassId(circleClass.getId(),0);
			}
			state=true;
		} catch (Exception e) {
			log.error("新增消息失败",e);
