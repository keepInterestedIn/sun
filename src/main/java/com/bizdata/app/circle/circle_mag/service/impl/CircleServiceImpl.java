...
public class CircleServiceImpl implements CircleService {
	...
	public boolean save(Circle circleClass) {
		...
		try {
			...
			if(circleClass.getState()==0){
				circleTopicService.setStateByCircleId(circleClass.getId(), "3");
			}
			state=true;
		} catch (Exception e) {
			log.error("新增消息失败",e);
	...
	@Transactional
	@Override
	public void setTopicnum(String circleeeid) {
		Circle cir = circleClassRepository.findOne(circleeeid);
		if(null==cir.getTopicnum()) {
			cir.setTopicnum(1);
			save(cir);
		}else {
			circleClassRepository.setTopicnum(circleeeid);
		}
	}
	//circle_id查询
	...
	@Override
	public void setStateByClassId(String classid, int state) {
		circleClassRepository.setStateByClassId(classid, state);
	}
