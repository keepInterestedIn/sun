...
public interface CircleRepository extends JpaBaseRepository<Circle, String> {
	...
	@Modifying
	void setTopicnum(String circleeeid);
	@Query(value="select * from app_circle where circle_id = ?1",nativeQuery=true)
	Circle findByCircleid(int circleId);
	@Modifying
	@Transactional
	@Query(value="update app_circle set state = ?2 where circleclassid = ?1",nativeQuery=true)
	void setStateByClassId(String classid, int state);
