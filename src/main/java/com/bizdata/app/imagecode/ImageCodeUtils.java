...
public class ImageCodeUtils {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ImageCode ic = new ImageCode(random,s,request.getSession().getId());
        //7.将随机数存在session中
        ...
        //删除该session已请求的验证码
        imageCodeRepository.deleteBySessionid(request.getSession().getId());
        imageCodeRepository.save(ic);
    }

    public void deleteBySessionid(String sessionid){
    	imageCodeRepository.deleteBySessionid(sessionid);
    }
    public ImageCode ckeckCode(String code,String sessionid){
    	long s = Calendar.getInstance().getTimeInMillis();
    	ImageCode c = imageCodeRepository.findOne(code,s-60000,s,sessionid);
    	return c;
    }
    /**
     * 设置图片的背景色
    ...
    private String createRandomChar(Graphics2D g,String baseChar) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sb.append(ch);
            // 正向角度
            g.rotate(degree * Math.PI / 180, x, 24);
	...
	public void delete(ImageCode c) {
		imageCodeRepository.delete(c);
	}
}
